...
def get_author(author_id: str,
               author_repo: AuthorCRUDRepositoryGraph = Depends(get_repository(repo_type=AuthorCRUDRepositoryGraph))):
    """
    Get a author by id
    """
...
@router.get("/search/{text}",
            name="author:search2")
def get_authors_by_title(text: str,
                       skip: int = 0,
                       limit: int = 3,
                       author_repo: AuthorCRUDRepositoryGraph = Depends(get_repository(repo_type=AuthorCRUDRepositoryGraph))):
    """
    ...
    """
    result = author_repo.search_authors_by_name(text=AuthorName(name=text).name, skip=skip, limit=limit)
    return result
