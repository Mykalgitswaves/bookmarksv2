...
from src.models.schemas.setup_user import (
    SetupUserFullName,
    SetupUserGenres,
    SetupUserAuthors,
)
...
@router.put("/full_name", name="setup-user:full_name")
async def setup_user_full_name(
    request: Request,
    ...
    if current_user:
        try:
            ...
            user = user_repo.update_user_full_name(
                username=current_user.username, full_name=setup_user_obj.full_name
            )
...
@router.put("/genres", name="setup-user:genres")
async def setup_user_genres(
    request: Request,
    current_user: Annotated[User, Depends(get_current_active_user)],
    user_repo: UserCRUDRepositoryGraph = Depends(
        get_repository(repo_type=UserCRUDRepositoryGraph)
    ...
    try:
        ...
        for genre in setup_user_genres.genres:
            ...
            response = user_repo.update_user_liked_genre(
                genre_id=genre, username=current_user.username
            )
            if not response:
                # Return HTTPException with status code 404 if the genre or user is not found
                return HTTPException(status_code=404, detail="Genre or User not found")
    except:
...
@router.put("/authors", name="setup-user:authors")
async def setup_user_authors(
    request: Request,
    current_user: Annotated[User, Depends(get_current_active_user)],
    user_repo: UserCRUDRepositoryGraph = Depends(
        get_repository(repo_type=UserCRUDRepositoryGraph)
    ...
    try:
        ...
        for author in setup_user_authors.authors:
            ...
            response = user_repo.update_user_liked_author(
                author_id=author, username=current_user.username
            )

            # If the response is False, return HTTPException with status code 404
            if not response:
                return HTTPException(status_code=404, detail="Author or User not found")
    except:
