...
class BookshelfWSManager:
    ...
    async def reorder_books_and_send_updated_data(self, bookshelf_id, data, bookshelf_repo:BookshelfCRUDRepositoryGraph):
        try:
            self.cache[bookshelf_id].reorder_book(**data.dict())
            books = jsonable_encoder(self.cache[bookshelf_id].get_books())
            bookshelf_repo.update_books_in_bookshelf(books=books, bookshelf_id=bookshelf_id)
